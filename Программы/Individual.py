...
Для своего индивидуального задания лабораторной работы 2.23 организован
...
отдельном потоке. Потоки для вычисления значений двух функций запускаются одновременно.

ПРИМЕЧАНИЕ: в модуле threading найден метод Barrier. Он предназначен для алгоритмов, в которых
для выполнения задачи необходимо дождаться завершения работы группы потоков.
P.S.Другой частично подходящий метод - Event, синхронизирующий операции потоков.

Индивидуальное задание из работы 2.23:
...
def second_var(results, barrier):
    ...
    with lock:
        results["second"] = s
    # Ожидание второго потока на барьере
    barrier.wait()


def third_var(results, barrier):
    ...
    with lock:
        results["third"] = s
    # Ожидание первого потока на барьере
    barrier.wait()
...
def main():
    ...
    # Создание барьера для синхронизации двух потоков.
    barrier = Barrier(2)

    # Создание потоков для вычисления сумм.
    first_thread = Thread(target=second_var, args=(results, barrier))
    second_thread = Thread(target=third_var, args=(results, barrier))
